...
public class StrawberrySmoothieFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        strawberry_smoothie_btn.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View view) {
                Intent intent = new Intent(getActivity(), SelectModeActivity.class);
                intent.putExtra("menu", "딸기스무디");
                intent.putExtra("price", "3300");
                intent.putExtra("temp", "아이스");
                startActivity(intent);

                FragmentManager fragmentManager = getActivity().getSupportFragmentManager();
                fragmentManager.beginTransaction().remove(StrawberrySmoothieFragment.this).commit();
    ...
    public void onResume() {
        ...
        tts = new TextToSpeech(getActivity(), new TextToSpeech.OnInitListener() {
            ...
            public void onInit(int status) {
                strawberry_smoothie_btn.setEnabled(true);
                Locale locale = Locale.getDefault();
                tts.setLanguage(locale);
