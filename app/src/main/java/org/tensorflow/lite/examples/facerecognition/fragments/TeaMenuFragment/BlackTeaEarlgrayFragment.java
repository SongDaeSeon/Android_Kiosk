...
public class BlackTeaEarlgrayFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        black_tea_earlgray_btn.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View view) {
                Intent intent = new Intent(getActivity(), SelectIceHotActivity.class);
                intent.putExtra("menu", "홍차얼그레이");
                intent.putExtra("price", "2000");
                startActivity(intent);

                FragmentManager fragmentManager = getActivity().getSupportFragmentManager();
                fragmentManager.beginTransaction().remove(BlackTeaEarlgrayFragment.this).commit();
    ...
    public void onResume() {
        ...
        tts = new TextToSpeech(getActivity(), new TextToSpeech.OnInitListener() {
            ...
            public void onInit(int status) {
                black_tea_earlgray_btn.setEnabled(true);
                Locale locale = Locale.getDefault();
                tts.setLanguage(locale);
