...
public class ChocoMuffinFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        choco_muffin_btn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                vibrator = (Vibrator) getActivity().getSystemService(Context.VIBRATOR_SERVICE);
                vibrator.vibrate(100); // 0.1초간 진동
                String text = "초코머핀 1800원";
        ...
        choco_muffin_btn.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View view) {
                Intent intent = new Intent(getActivity(), SelectModeActivity.class);
                intent.putExtra("menu", "초코머핀");
                intent.putExtra("price", "1800");
                intent.putExtra("temp","없음");
                startActivity(intent);

                FragmentManager fragmentManager = getActivity().getSupportFragmentManager();
                fragmentManager.beginTransaction().remove(ChocoMuffinFragment.this).commit();
    ...
    public void onResume() {
        ...
        tts = new TextToSpeech(getActivity(), new TextToSpeech.OnInitListener() {
            ...
            public void onInit(int status) {
                choco_muffin_btn.setEnabled(true);
                Locale locale = Locale.getDefault();
                tts.setLanguage(locale);
