...
public class ChocoMousseCakeFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        choco_mousse_cake_btn.setOnClickListener(new View.OnClickListener() {
            @Override
        ...
        choco_mousse_cake_btn.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View view) {
                Intent intent = new Intent(getActivity(), SelectModeActivity.class);
                intent.putExtra("menu", "초코무스조각케익");
                intent.putExtra("price", "2500");
                intent.putExtra("temp","없음");
                startActivity(intent);

                FragmentManager fragmentManager = getActivity().getSupportFragmentManager();
                fragmentManager.beginTransaction().remove(ChocoMousseCakeFragment.this).commit();
    ...
    public void onResume() {
        ...
        tts = new TextToSpeech(getActivity(), new TextToSpeech.OnInitListener() {
            ...
            public void onInit(int status) {
                choco_mousse_cake_btn.setEnabled(true);
                Locale locale = Locale.getDefault();
                tts.setLanguage(locale);
